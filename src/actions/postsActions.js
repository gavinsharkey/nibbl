...
const fetchFeed = () => {
  return dispatch => {
    ...
    fetchWithCredentials(`http://nibbl.live/api/v1/posts?page=0`)
    .then(json => {
...
const fetchMoreFeed = page => {
  return dispatch => {
    ...
    fetchWithCredentials(`http://nibbl.live/api/v1/posts?page=${page}`)
    .then(json => {
      if (json.length === 0) {
...
const fetchPostsByUser = (userId) => {
  return dispatch => {
    dispatch({type: 'LOADING_POSTS'})
    fetchWithCredentials(`http://nibbl.live/api/v1/posts?page=0&user_id=${userId}`)
    .then(json => {
      dispatch({type: 'SET_POSTS', posts: json})
    })
  }
...
const fetchMorePostsByUser = (userId, page) => {
  return dispatch => {
    dispatch({type: 'LOADING_MORE_POSTS'})
    fetchWithCredentials(`http://nibbl.live/api/v1/posts?page=${page}&user_id=${userId}`)
    .then(json => {
      if (json.length === 0) {
        dispatch({type: 'NO_MORE_POSTS'})
      } else {
...
const createPost = content => {
  return dispatch => {
    fetchWithCredentials('http://nibbl.live/api/v1/posts', 'POST', {
      post: {
        content
      }
    })
...
const likePost = postId => {
  return dispatch => {
    ...
    fetchWithCredentials('http://nibbl.live/api/v1/likes', 'POST', { post_id: postId })
    .then(json => {
      if (!json.errors) {
...
const unlikePost = likeId => {
  return dispatch => {
    dispatch({type: 'LOADING_LIKE'})
    fetchWithCredentials(`http://nibbl.live/api/v1/likes/${likeId}`, 'DELETE')
    .then(json => {
