...
const fetchFeed = () => {
  return dispatch => {
    ...
    fetchWithCredentials(`https://powerful-springs-89951.herokuapp.com/api/v1/posts?page=0`)
    .then(json => {
...
const fetchMoreFeed = page => {
  return dispatch => {
    ...
    fetchWithCredentials(`https://powerful-springs-89951.herokuapp.com/api/v1/posts?page=${page}`)
    .then(json => {
      if (json.length === 0) {
...
const fetchPostsByUser = (userId) => {
  return dispatch => {
    dispatch({type: 'LOADING_POSTS'})
    fetchWithCredentials(`https://powerful-springs-89951.herokuapp.com/api/v1/posts?page=0&user_id=${userId}`)
    .then(json => {
      dispatch({type: 'SET_POSTS', posts: json})
    })
  }
...
const fetchMorePostsByUser = (userId, page) => {
  return dispatch => {
    dispatch({type: 'LOADING_MORE_POSTS'})
    fetchWithCredentials(`https://powerful-springs-89951.herokuapp.com/api/v1/posts?page=${page}&user_id=${userId}`)
    .then(json => {
      if (json.length === 0) {
        dispatch({type: 'NO_MORE_POSTS'})
      } else {
...
const createPost = content => {
  return dispatch => {
    fetchWithCredentials('https://powerful-springs-89951.herokuapp.com/api/v1/posts', 'POST', {
      post: {
        content
      }
    })
