...
const fetchUser = userId => {
  return dispatch => {
    ...
    fetchWithCredentials(`https://powerful-springs-89951.herokuapp.com/api/v1/users/${userId}`)
    .then(json => {
      if (json.status === 404) {
...
const updateUser = (userId, data) => {
  return dispatch => {
    dispatch({type: 'LOADING_USER'})
    fetchWithCredentials(`https://powerful-springs-89951.herokuapp.com/api/v1/users/${userId}`, 'PATCH', {
      user: {
